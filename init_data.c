...
//	printf("writing xpm file\n");
	obj[i].tile[0].ptr = mymlx_xpm_file_to_image
		(g->mlx_ptr,
		tile,
		&obj[i].tile[0].w,
		&obj[i].tile[0].h
...
//	printf("coppying xpm adress\n");
	obj[i].tile[0].data_ptr = (int *)mymlx_get_data_addr
		(
		obj[i].tile[0].ptr,	
		&obj[i].tile[0].bpp,
		&obj[i].tile[0].sz_l,
...
//(t_vector *)do_tile_sphere(t_vector, t_object *, t_global *);
	...
	init_vector(g->obj[i].ctr, 0, -100, 300);
	printf("center is %f\n", g->obj[i].ctr->z);
	g->obj[i].rd = 100;
	g->obj[i].rd2 = g->obj[i].rd * g->obj[i].rd;
	g->obj[i].color = rgb(0x010100);
...
/*
	...
	printf("hello999\n");
	g->obj[i].tile[0].data_ptr = (int *)mymlx_get_data_addr
		(
		g->obj[i].tile[0].ptr,	
		&g->obj[i].tile[0].bpp,
		&g->obj[i].tile[0].sz_l,
